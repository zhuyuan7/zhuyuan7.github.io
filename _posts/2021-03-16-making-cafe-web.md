...
이러한 지도를 만들기 위해 카카오맵 API의 [다각형에 이벤트 등록하기2](https://apis.map.kakao.com/web/sample/addPolygonMouseEvent2/)를 
이용하여 서울시 25개 구 지도를 구현하였다. 
API가 제공하는 좌표주소를 통해 "구"단위를 나누는 코드를 사용하여 "구"지도를 구현하였고,
카카오맵에서 기본적으로 제공하는 좌표는 8개 "구"(용산구, 중구, 종로구, 서대문구, 동대문구,
성북구, 성동구, 마포구) 이기 때문에 
8개 "구"를 제외한 나머지 구의 좌표값은 내가 직접 구해야만 했다.

카카오맵 API에서 제공한 코드가 "구"단위 좌표값으로 되어있기 때문에 
...
부동산 사이트의 지도이다. 부동산 사이트가 지도를 잘 활용할 것 같아 
"구"단위로 부동산을 알아볼 수 있게 만들어놓은 
**네이버 부동산**에서 17개 "구"의 좌표값을 구했다.   

그 결과, 다음과 같은 서울특별시 25개 "구"를 구현하였다.
